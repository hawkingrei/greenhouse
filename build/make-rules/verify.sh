...
BFS_ROOT=$(dirname "${BASH_SOURCE}")/../..
source "${BFS_ROOT}/hack/lib/util.sh"

# include shell2junit library
# source "${BFS_ROOT}/third_party/forked/shell2junit/sh2ju.sh"
...
EXCLUDED_CHECKS=$(ls ${EXCLUDED_PATTERNS[@]/#/${BFS_ROOT}\/hack\/} 2>/dev/null || true)
QUICK_CHECKS=$(ls ${QUICK_PATTERNS[@]/#/${BFS_ROOT}\/hack\/} 2>/dev/null || true)
...
function run-cmd {
  ...
  local output="${BFS_JUNIT_REPORT_DIR:-/tmp/junit-results}"
  local tr
...
ret=0
run-checks "${BFS_ROOT}/hack/verify-*.sh" bash
run-checks "${BFS_ROOT}/hack/verify-*.py" python
